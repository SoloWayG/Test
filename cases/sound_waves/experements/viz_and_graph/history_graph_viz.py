...
ls = os.listdir(path='.')
lenght = 1#count_files(path ='.', like ='optimized_structure_')
archs = count_files(path ='./History', like ='performance_')
pwd = os.getcwd()
...
for a in range(lenght):
    performance_path_2 = ([f"History/performance_{i}.pickle" for i in range(archs)])
    stractures = ([upload_file(f"History/population_{i}.pickle") for i in range(archs)])
    fitness = ([upload_file(i) for i in performance_path_2])
for r in range(len(fitness)):
